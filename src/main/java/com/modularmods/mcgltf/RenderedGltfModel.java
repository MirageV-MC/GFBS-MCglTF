...
import org.Convex89524.gfbs_mcgltf.GFBsMCglTF;
...
public class RenderedGltfModel {
	...
	protected static final Runnable vanillaDefaultMaterialCommand = () -> {
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, GFBsMCglTF.getInstance().getDefaultColorMap());
	...
	protected static final Runnable shaderModDefaultMaterialCommand = () -> {
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, GFBsMCglTF.getInstance().getDefaultNormalMap());
		GL13.glActiveTexture(SPECULAR_MAP_INDEX);
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, GFBsMCglTF.getInstance().getDefaultSpecularMap());
		GL20.glVertexAttrib4f(vaColor, 1.0F, 1.0F, 1.0F, 1.0F);
		GL11.glEnable(GL11.GL_CULL_FACE);
	};
		...
		public void initMaterialCommand(List<Runnable> gltfRenderData, RenderedGltfModel renderedModel, MaterialModel materialModel) {
			...
			if(materialModel instanceof MaterialModelV2) {
				...
				if(baseColorTexture == null) {
					...
					else colorMap = GFBsMCglTF.getInstance().getDefaultColorMap();
				}
				...
				if(normalTexture == null) {
					...
					else normalMap = GFBsMCglTF.getInstance().getDefaultNormalMap();
				}
				...
				if(specularTexture == null) {
					...
					else specularMap = GFBsMCglTF.getInstance().getDefaultSpecularMap();
				}
				...
				colorMap = baseColorTexture == null ? GFBsMCglTF.getInstance().getDefaultColorMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(baseColorTexture.index));
				normalMap = normalTexture == null ? GFBsMCglTF.getInstance().getDefaultNormalMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(normalTexture.index));
				specularMap = specularTexture == null ? GFBsMCglTF.getInstance().getDefaultSpecularMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(specularTexture.index));
	...
	public int obtainGlTexture(List<Runnable> gltfRenderData, TextureModel textureModel) {
		...
		if(glTexture == null) {
			...
			if (pixelData == null)
			{
				GFBsMCglTF.logger.warn("Could not extract pixel data from image");
